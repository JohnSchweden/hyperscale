...
<objective>
Integrate 80+ new no-win scenario cards into the game loop, wire pressure metadata for ~20% of high-stakes cards, and validate that deck shuffle includes all new content without breaking existing functionality.

Purpose: Ensure new cards are live in game (no dead code), time pressure is calibrated for impact (not fatigue), and all systems (shuffle, pressure metadata, haptics, feedback) work cohesively.

Output: Updated game state, pressure metadata mapping, E2E tests validating card variety and pressure integration.
</objective>
...
Heuristic: If outcome involves prison, audit failure, major budget burn, or public scandal → mark urgent.

Example cards to mark urgent:
- "Customer discovered injection bug" (PROMPT_INJECTION variant)
- "Auditor demands explainability" (EXPLAINABILITY variant)
- "Model drift costs $500K to fix" (DRIFT variant)
- "Copyright lawsuit pending" (COPYRIGHT variant)
- "Team burnout detected" (TEAM_SAFETY variant)
...
Phase 03 adds 80+ cards to ROLE_CARDS. Shuffle logic automatically includes them (no changes needed to shuffle function).

**Validation:** Play 5+ games, verify different card orders.
...
- All 80+ new cards have 3-voice feedback (created in Plan 02)
...
Phase 03 Plan 03 task: Define PRESSURE_METADATA with entries for ~20% of cards (high-stakes).
...
Phase 03 validates:
- New cards included in shuffle
- Pressure metadata wired for correct cards (not all cards)
- E2E: Play full game, verify card variety in history
- No regression in existing card playability

## Card Sourcing Metadata (from Plan 02)

tests/data/card-sources.json populated with:
- cardId → incident source, date, category, reference URL
...
<task type="auto" tdd="true">
  <name>Task 1: Create data/pressure-metadata.ts — Map ~20% of cards to pressure scenarios</name>
  ...
  <behavior>
    ...
    - Categories for urgent cards: security breach, legal escalation, budget cliff, team impact, reputation crisis
    - All other cards have empty entry or omitted (optional metadata)
  </behavior>
  <action>
...
export const PRESSURE_METADATA: Record<string, PressureScenarioMetadata> = {
  // Prompt Injection — public disclosure variant (customer discovers bug)
  "prompt_injection_dev_02": {
    urgent: true,
    countdownSec: 45,
    timeoutResolvesTo: "LEFT",  // Default: try to fix quietly (escalates pressure)
    criticalForHaptics: true,
    outcomes: {
      LEFT: { teamImpact: "Security team rallies for damage control" },
      RIGHT: { teamImpact: "Customers discover the breach via social media" },
    },
  },

  // Copyright lawsuit scenario
  "copyright_finance_02": {
    urgent: true,
    countdownSec: 60,
    timeoutResolvesTo: "RIGHT",  // Default: defer audit (legal exposure builds)
    criticalForHaptics: true,
    outcomes: {
      LEFT: { teamImpact: "Legal team works overtime on discovery" },
      RIGHT: { teamImpact: "Lawsuit surfaces in weeks, damages increase" },
    },
  },

  // Model drift with major budget impact
  "model_drift_finance_01": {
    urgent: true,
    countdownSec: 40,
    timeoutResolvesTo: "RIGHT",  // Default: freeze (revenue loss escalates)
    criticalForHaptics: true,
    outcomes: {
      LEFT: { teamImpact: "ML team pulls all-nighters on retraining" },
      RIGHT: { teamImpact: "Revenue hemorrhages as customers lose confidence" },
    },
  },

  // Audit escalation
  "explainability_management_01": {
    urgent: true,
    countdownSec: 50,
    timeoutResolvesTo: "RIGHT",  // Default: defer (audit risk)
    criticalForHaptics: true,
    outcomes: {
      LEFT: { teamImpact: "Engineering team reassigned to compliance work" },
      RIGHT: { teamImpact: "Auditor escalates to C-suite, board pressure" },
    },
  },

  // Team burnout / safety scenario
  "team_safety_management_01": {
    urgent: true,
    countdownSec: 35,
    timeoutResolvesTo: "LEFT",  // Default: push harder (burnout escalates)
    criticalForHaptics: true,
    outcomes: {
      LEFT: { teamImpact: "Team works 16-hour days, morale tanks" },
      RIGHT: { teamImpact: "Stakeholders blame team for delay, no relief" },
    },
  },

  // ... 10-13 more high-stakes cards (security, legal, budget, reputation, team impact)
...
**Urgent card selection heuristic:**
- Security incident with public disclosure (customer impact) → urgent
- Legal/audit discovery (regulatory timeline) → urgent
- Budget cliff ($500K+) → urgent
- Team safety/ethics violation → urgent
- Reputation crisis (public scandal) → urgent

**Not urgent** (but important):
- Internal discovery (time to fix)
- Optimization questions (no immediate crisis)
- Generic governance dilemmas (no external pressure)
- Routine compliance (planned timeline)

**countdownSec calibration:**
- Security breach: 45-60 sec (high stakes, short decision window)
- Budget decision: 40-60 sec (finance needs to decide quickly)
- Team safety: 30-45 sec (morale crisis accelerates)
- Audit/legal: 50-70 sec (regulatory pressure is imminent)

**timeoutResolvesTo logic:**
- If timeout → LEFT = "conservative choice" (delay, audit, safety)
- If timeout → RIGHT = "aggressive choice" (deploy, risk, speed)
- Match the card's structure: which path escalates pressure if chosen by default?

Example: Prompt injection with customer discovery:
- LEFT: "Fix quietly" (conservative, buys time, leads to escalation if default)
- RIGHT: "Public disclosure" (aggressive, transparency, immediate fallout)
- timeoutResolvesTo: "LEFT" (pressure increases if player defaults to quiet fix)
...
**Validation:** Count cards in PRESSURE_METADATA, should be ~15-18 (20% of 80+). Run tests.
  ...
  <done>pressure-metadata.ts created with 15-18 urgent cards. All mapped to PressureScenarioMetadata. Validation passes.</done>
</task>

<task type="auto" tdd="true">
  <name>Task 2: Update useGameState.ts — Include new cards in shuffle, lookup pressure metadata</name>
  <files>hooks/useGameState.ts</files>
  <behavior>
    - On PLAYING stage initialization: shuffle includes all 80+ cards (no changes to shuffle function needed)
    - Verify ROLE_CARDS now points to expanded decks (10+ cards per role)
...
1. Verify ROLE_CARDS import points to updated data/cards/index.ts with expanded decks
2. In INITIALIZING → PLAYING transition, shuffle deck as before (no change needed):
   ```typescript
   case "INITIALIZE":
     const deck = shuffleArray(ROLE_CARDS[payload.role]); // Already includes 80+ cards
...
This wires pressure data through the game state. Pressure hooks (useIncidentPressure, etc.) will read currentCardPressure from state.
  </action>
  <verify>
    <automated>npm run build 2>&1 | grep -i error | head -5</automated>
  </verify>
  <done>useGameState.ts updated. Pressure metadata wired into state. Build passes. No TypeScript errors.</done>
</task>

<task type="auto" tdd="true">
  <name>Task 3: Verify CardStack.tsx renders all new card data correctly</name>
  <files>components/CardStack.tsx</files>
  <behavior>
    - CardStack already renders card.text, source, sender, storyContext (from Phase 12)
    ...
    - No new features needed; just validate existing rendering handles 80+ new cards
  </behavior>
  <action>
...
2. Verify pressure UI:
   ...
   - Team impact text renders from FeedbackOverlay (not CardStack)
   - No new UI needed
...
No code changes expected; just validation that existing rendering works with new card data.
  </action>
  <verify>
    <automated>grep -n "card\\." /Users/yevgenschweden/swiperisk/components/CardStack.tsx | head -10</automated>
  </verify>
  <done>CardStack.tsx verified. Renders all new card fields correctly. No layout issues with longer scenarios.</done>
</task>

<task type="auto" tdd="true">
  <name>Task 4: Verify App.tsx pressure hook integration</name>
  <files>App.tsx</files>
  <behavior>
    ...
    - Verify time pressure + haptics work with new cards (no regression)
  </behavior>
  <action>
Audit `App.tsx`:
...
4. No changes expected if Phase 04 wiring is complete. Just validation.
  </action>
  <verify>
  ...
  <done>App.tsx verified. Pressure hooks wired. currentCardPressure flows to all components. No changes needed.</done>
</task>

<task type="auto" tdd="true">
  <name>Task 5: Create E2E test — Validate card deck shuffle and pressure integration</name>
  <files>tests/e2e/card-deck.spec.ts</files>
  <behavior>
    - Test 1: Play full game, verify 80+ cards in deck (no duplicates in shuffle)
    - Test 2: Play 3+ games, verify different card orders (shuffle is working)
...
test.describe("Card Deck Integration", () => {
  test("should include all new cards in shuffled deck", async ({ page }) => {
    // Play a full game
    await page.goto("http://localhost:5173"); // Adjust URL

    // Select personality and role
    await page.click('button:has-text("ROASTER")');
    await page.click('button:has-text("SOFTWARE_ENGINEER")');

    // Play all cards until game over
    const cardIds = [];
    while (!page.locator('text=GAME OVER').isVisible({ timeout: 500 })) {
      const cardId = await page.locator('[data-card-id]').getAttribute('data-card-id');
      if (cardId) cardIds.push(cardId);

      // Make a decision (swipe right)
      await page.swipe({ x: 300, y: 400 }, { x: 100, y: 400 });
      await page.waitForTimeout(500);
    }

    // Verify cards were played (at least some cards)
  ...
  test("should randomize deck order across games", async ({ page, context }) => {
    ...
    const allSame = gameDecks.every(deck => JSON.stringify(deck) === JSON.stringify(gameDecks[0]));
  ...
  test("should display timer for urgent cards", async ({ page }) => {
    ...
    // Play until an urgent card appears
    let foundUrgent = false;
    while (!page.locator('text=GAME OVER').isVisible({ timeout: 500 })) {
      const timerVisible = await page.locator('[data-test="countdown-timer"]').isVisible();
      if (timerVisible) {
        foundUrgent = true;
        // Verify timer counts down
        const initialValue = await page.locator('[data-test="countdown-timer"]').textContent();
        await page.waitForTimeout(1000);
        const newValue = await page.locator('[data-test="countdown-timer"]').textContent();
        expect(newValue).not.toBe(initialValue);
        break;
      }

      await page.swipe(...); // Advance game
    }

    // Note: May or may not find urgent card (20% chance)
    // Test is non-blocking if no urgent card encountered
  });
  ...
  test("should not display timer for non-urgent cards", async ({ page }) => {
    ...
    for (let i = 0; i < 3; i++) {
      const timerVisible = await page.locator('[data-test="countdown-timer"]').isVisible();
      expect(timerVisible).toBe(false); // First cards are usually non-urgent
      await page.swipe(...);
    }
  });
});
...
**Note:** E2E tests may be flaky due to random shuffle. Use high timeout values and non-blocking assertions for optional elements (timer may not appear on every run).
...
Update CardStack.tsx to add these attributes if missing.
  ...
  <done>E2E tests created. Validates shuffle, pressure timer, incident variety. Tests passing (may have timeout warnings, acceptable for flaky shuffle tests).</done>
</task>
...
<task type="auto">
  <name>Task 7: Run full test suite — Regression validation</name>
  <files></files>
  <action>
...
- E2E tests pass (deck shuffle works, pressure integration works)
- Data tests pass (all 80+ cards valid)
...
- Shuffle includes duplicates → verify shuffleArray works (shouldn't change)
- Pressure metadata not wired → verify useGameState passes to CardStack
  ...
  <done>All tests passing. No regressions. Game integration complete.</done>
</task>

<task type="auto">
  <name>Task 8: Git commit — Pressure metadata integration and E2E tests</name>
  <files></files>
  <action>
...
git commit -m "feat(03): integrate pressure metadata and validate card deck

- Create pressure-metadata.ts: 15-18 high-stakes cards marked urgent
  - Security breach with public disclosure
  - Legal escalation and audit pressure
  - Budget cliff decisions ($500K+)
  - Team safety and burnout scenarios
  - Reputation/ethics crises
  - Timer: 30-70 seconds depending on scenario
...
Tests passing: Unit, E2E, Data validation. Ready for UAT (Phase 03 Plan 04)."
```
  </action>
  <verify>
    <automated>git log --oneline | head -1</automated>
  </verify>
  <done>Integration commit complete. All tests passing. Phase 03 ready for UAT.</done>
</task>

</tasks>

<verification>
1. All 80+ new cards integrated into ROLE_CARDS
2. Deck shuffle includes new cards (tested E2E)
...
6. E2E tests validate shuffle randomization and pressure integration
7. All tests passing (unit, E2E, data)
8. Git commit shows integration complete
</verification>

<success_criteria>
- [ ] PRESSURE_METADATA.ts created with 15-18 urgent cards
- [ ] useGameState.ts wired with pressure metadata lookup
- [ ] CardStack.tsx renders all new card fields (no layout issues)
- [ ] App.tsx pressure hooks integrated (no changes needed)
- [ ] E2E tests validate shuffle + pressure timer + incident variety
...
- [ ] Deck includes all 80+ cards on game start (shuffle verified)
- [ ] Urgent cards trigger timer UI (~20% of deck)
- [ ] Commit shows integration complete
</success_criteria>

<output>
After completion, create `.planning/phases/03-no-win-scenario-cards/03-03-SUMMARY.md` with:
- Pressure metadata mapping summary (15-18 urgent cards)
- Integration validation (shuffle, pressure wiring, E2E tests)
- Test results (all suites passing, no regressions)
- Next step: Phase 03 Plan 04 (UAT and final verification)
</output>
